...
\begin{tikzpicture}[node distance=0.5cm and 0.4cm, scale=0.88, transform shape]
  ...
  \node[system, minimum width=9.5cm, minimum height=13cm,
        label={[font=\bfseries\small, vfgreen]above:Hệ thống VietForces (Android App)}]
        (sysbox) at (4.75, -6) {};
  ...
  % --- include / extend ---
  \draw[dar] (play) -- node[above,font=\tiny]{«include»} (elo);
  \draw[dar] (daily) -- node[above,font=\tiny]{«include»} (streak);
  \draw[dar] (login) -- node[above,font=\tiny]{«extend»} (onboard);
  \draw[dar] (social) -- node[above,font=\tiny]{«extend»} (profile);

  % --- Actor: Admin (hộp web admin riêng) ---
  \node[system, minimum width=6.5cm, minimum height=5.5cm,
        label={[font=\bfseries\small, vfgreen]above:Web Admin Dashboard},
  ...
  \foreach \uc in {voccrud, usermgmt, analytics, sched}
    \draw[ar] (admin) -- (\uc);
  \draw[dar] (usermgmt) -- node[right,font=\tiny]{«extend»} (ban);
...
\midrule
Onboarding & Người học & Màn hình chào mừng 4 bước (Welcome → Chọn level → Đặt mục tiêu → Nhập tên/avatar) trước khi tạo tài khoản. \\
\midrule
...
\midrule
ELO Rank Tier & Hạng được xác định dựa trên ELO: \textbf{Bronze} (<1200), \textbf{Silver} (1200–2099), \textbf{Gold} (2100–2399), \textbf{Platinum} (2400–2699), \textbf{Diamond} (≥2700). \\
\midrule
...
% ---- GameMode enum ----
\node[enh] (gmhdr) at (7.5,-0) {«enum» GameMode};
...
% ---- EloRank enum ----
\node[enh, below=1cm of gmbody] (erhdr) {«enum» EloRank};
...
\begin{enumerate}
  ...
  \item \textbf{Gamification đầy đủ:}
    \begin{itemize}
      ...
      \item ELO ranking với 5 tier (Bronze → Diamond).
    \end{itemize}
